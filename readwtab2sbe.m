function [sbeG,sbeNode,retFname]=readwtab2sbe(filename,usenodenum,casesensitive)
...
dataCell = textscan(tab_file_id,'%s%s%f%*[^\n\r]', 'delimiter', '\t');
...
if ~usenodenum
    ...
    sbeG=zeros(n);
    for k=1:numel(x)/2
        sbeG(y(k,1),y(k,2))=dataCell{3}(k);
        sbeG(y(k,2),y(k,1))=dataCell{3}(k);
    end
else
    n=max(c(:));
    if min(c)~=1
    ...
    for k=1:length(dataCell{1})
        sbeG(dataCell{1}(k),dataCell{2}(k))=dataCell{3}(k);
        sbeG(dataCell{2}(k),dataCell{1}(k))=dataCell{3}(k);
    end
end
